...
In the following predictions we notice that the network is very accuarate by detecting the rear of the cars when the camera only see cars on the same direction. However when the street is two ways and the bounding box is in the right of the image there are missclassifications as expected. 
